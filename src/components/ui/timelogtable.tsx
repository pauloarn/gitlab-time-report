            ...
            return (
              <React.Fragment key={index}>
                {Array.from({ length: maxRows }).map((_, rowIndex) => (
                  <tr key={`${index}-${rowIndex}`} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    {rowIndex === 0 && (
                      <>
                        <td
                          rowSpan={maxRows}
                          className="w-1/4 px-4 py-4 text-sm font-medium text-gray-900 dark:text-gray-100 break-words align-top"
                        >
                          {item.webUrl ? (
                            <a
                              href={item.webUrl}
                        ...
                        {issueLevelColumns.map((col, colIndex) => (
                          <td
                            ...
                            rowSpan={maxRows}
                            className="px-4 py-4 text-sm text-gray-900 dark:text-gray-100 break-words align-top"
                          >
                            {col.render
                    ...
                    {subItemColumns.map((itemKey, colIndex) => {
                      ...
                      return (
                        <td
                          ...
                          className="w-1/2 px-4 py-4 text-sm font-medium text-gray-900 dark:text-gray-100 break-words"
                        >
                          {subItem ? (
